...
local MASTER_DB = {
    ...
    { id=55048, name="Elixir of Greater Arcane Power", icon="inv_potion_81" },     
    ...
    { id=9224,  name="Elixir of Demonslaying", icon="inv_potion_27" },   
    ...
    { id=60978, name="Danonzo's Tel'Abim Medley", icon="inv_misc_food_73", bufficon="spell_misc_food" },
    { id=60977, name="Danonzo's Tel'Abim Delight", icon="inv_drink_21", bufficon="spell_misc_food" }, 
    { id=60976, name="Danonzo's Tel'Abim Surprise", icon="inv_misc_food_92", bufficon="spell_misc_food" },            
...
function UpdateTrackers()
    ...
    for catIndex, catData in ipairs(ConsumablesDB.categories) do
        ...
        if catData.enabled ~= false then 
            ...
            local isRightAligned = false
            if independent then
                if catData.alignRight == true then isRightAligned = true end
            else
                isRightAligned = settings.alignRight
            end
...
eventFrame:SetScript("OnEvent", function()
    if event == "VARIABLES_LOADED" then
        if not ConsumablesDB then
            ConsumablesDB = {
                settings = {
                    ...
                    showBackground=true, minimap={ hide=false, angle=45 }
                },
        ...
        CreateMinimapButton()
        UPDATE_QUEUED = true
    elseif event == "PLAYER_REGEN_DISABLED" or event == "PLAYER_REGEN_ENABLED" or event == "RAID_ROSTER_UPDATE" or event == "PLAYER_ENTERING_WORLD" or event == "BAG_UPDATE" then
        if event == "BAG_UPDATE" or event == "PLAYER_ENTERING_WORLD" then lastBagScan = 0 end
        UPDATE_QUEUED = true
