# Consumables!

Hello Turtles! 🤗

Have you ever thought to yourself what consumables do I need? What consumables do I have? What consumables have I currently got active, and how long do those consumables have before I need to reapply them? If so, this addon provides a comprehensive, dynamic answer to all of those questions. Introducing... 

🫒 **Consumables!** 🧪

💡Features: 💡

🛠️ - Create and name custom consumable tracking frames, and assign buffs/consumabes to individual groups. Many ways to configure, with individual specs, roles, or groups for each raid.

🧃 - Every SINGLE buff in the game (Pending any missing buffs, the list of which is growing smaller by the day).

🔑 - One click usage, no double consuming, will only use if not active.

🗂️ - CHANGE the order of your groups, or buffs within groups for maximum organization.

👀 - NO WASTED SPACE! Show or hide relevant groups, ensuring only relevant consumables are displayed.

🖱️ - Show on mouseover option!

⌨️ - Keybind show/hide for frames!

🚫 - Show/disable frame, show/hide background, or hide in combat.

🧑‍🤝‍🧑 - Group the consumables frames, or detach to separate groups, to allow maximum visual cohesion.

🔒 - Option to hide the frames when not in a raid.

📐 - Change the size, vertical/horizontal spacing, and rows of icons, as well as justification of the frames.

⚡ - Show/hide active buffs/consumes.

⚠️ - Flask warning, protection for accidental Flask usage.

⚙️ - Open/close configuration menu with /co.

🏎️ - Fast, lightweight addon!

🗺️ - MINI MAP BUTTON.

(Please note, all code is produced off the top of the dome, no AI was used, completely freestyle code, 100% natural, organic, olive oil code 🫒) PS: I have tested the addon, however it is likely that there may be bugs, or issues. Please notify me if you find them for immediate(ish) rectification.
